...
/** Ganesh Chaturthi. Override with the real muhurat for the year. */
...
export function festivalNow(at: number = Date.now()): FestivalNow {
  ...
  if (at < startsAt) {
    return { ...base, lifecycle: 'PRE_LAUNCH', day: 0, accepting: false };
  }

  if (at < endsAt) {
    const day = Math.min(FESTIVAL_DAYS, Math.floor((at - startsAt) / DAY_MS) + 1);
    return { ...base, lifecycle: `DAY_${day}` as Lifecycle, day, accepting: true };
  }
  ...
  if (at < endsAt + VISARJAN_WINDOW_MS) {
    return { ...base, lifecycle: 'VISARJAN', day: FESTIVAL_DAYS, accepting: false };
  }

  return { ...base, lifecycle: 'COMPLETED', day: FESTIVAL_DAYS, accepting: false };
}
